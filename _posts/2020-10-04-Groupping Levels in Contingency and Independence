...
$$\Chi^2= \sum_{i=1}^{I}\sum_{j=1}^{J} \frac{\left(n_{ij}-n_{i+}n_{+j} \right)^2}{ n_{i+}n_{+j}}$$
...
It can now be seen that: $$G_0 \geq G_1$$. Due to the condition of equality in the log-sum inequality, $$G_0=G_1$$ when the first two rows of $$M-0$$ are linearly dependent. 
