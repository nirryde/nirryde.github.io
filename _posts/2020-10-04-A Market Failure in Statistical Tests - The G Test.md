...
Contingency tables are not a sexy topic in statistics since much of the theory in the matter was developed a long time ago, when computers where essentially non existing. As a result, many methods for analyzing contingency tables were developed so that they could be used with a pen, a paper, and a suitable distribution table (such as the table of the standard normal distribution many students are familiar with<sup>1</sup>). As I expected, the course was not very interesting although there were a couple of interesting gems one of them is the subject of this post.
...
The rules for decomposing a contingency table into independent sub-tables are<sup>2</sup>:
...
Recall that the log-sum inequality states that for any sets of non-negative numbers: $$(x_i)_{i=1}^{n},(y_i)_{i=1}^{n}$$ with $$x=\sum_{i=1}^{n} x_i, y=\sum_{i=1}^{n} y_i$$ the following inequality holds<sup>3</sup>:
...
$$\frac{1}{2n_++}\left( G_0-G_1 \right) \geq \sum_{i=1}^{2} \left( \sum_{j=1}^{J}\pi_{ij} log \left( \frac{\pi_{ij}}{\pi_{1j}+\pi_{2j}} \right) - \pi_{i+} log \left( \frac{\pi_{i+}}{\pi_{1+}+\pi_{2+}} \right) \right) \geq 0$$  
